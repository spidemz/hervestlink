...
async function loadFarmer() {
  ...
  document.getElementById('orderValue').textContent = `$${orders.reduce((total, order) => total + Number(order.value), 0).toLocaleString()}`;
  document.getElementById('listingRows').innerHTML = data.produce.map(item => `<tr><td><strong>${item.name}</strong><small>${item.id}</small></td><td>${item.region}</td><td>${Number(item.quantity).toLocaleString()} kg</td><td>$${Number(item.price).toFixed(2)}</td><td><span class="status">${item.status}</span></td></tr>`).join('') || '<tr><td colspan="5">No listings found for this farm.</td></tr>';
